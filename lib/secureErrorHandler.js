...
class InputValidator {
  ...
  static validatePackageName(packageName) {
    ...
    const validLocalPath = /^[./]|^[a-zA-Z0-9._-]+\/[a-zA-Z0-9._-]+/;
    
    if (!validPackageName.test(packageName) && !validLocalPath.test(packageName)) {
  ...
  static validateScanOptions(options) {
    ...
    if (validatedOptions.output !== undefined) {
      ...
      if (!validFormats.includes(validatedOptions.output)) {
        throw new ValidationError(
  ...
  static validateFileContent(content, filename = 'unknown') {
    ...
    const maliciousPatterns = [
      ...
      /fs\s*\.(?:readFile|writeFile|unlink|rmdir)/gi,
      /fs\s*\.(?:readdir|stat|access)/gi,
