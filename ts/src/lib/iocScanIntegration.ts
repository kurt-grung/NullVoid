...
import type { IoCResult, IoCProviderName, IoCProviderConfig, IoCQueryOptions } from '../types/ioc-types';
...
function iocResultToThreat(iocResult: IoCResult, packageJsonPath?: string): Threat {
  ...
  if (iocResult.references.length > 0) {
    details += ` References: ${iocResult.references.slice(0, 2).map(ref => {
      // Shorten long URLs
      if (ref.length > 60) {
...
export async function queryIoCProviders(
  ...
  try {
    ...
    const aggregatedResults = await ioCManager.queryAll(
      queryOptions,
      providerNames
    );
    ...
    logger.debug(`IoC query for ${packageName}@${version || 'latest'} returned ${threats.length} threats`, {});
  } catch (error) {
    logger.warn(`IoC query failed for ${packageName}`, { error: error instanceof Error ? error.message : String(error) });
  }
 ...
 */
export function mergeIoCThreats(
  ...
  for (const threat of existingThreats) {
    const metadata = threat.metadata as { cve?: string; vulnerabilityId?: string; packageName?: string } | undefined;
    if (metadata?.cve) {
  ...
  for (const iocThreat of iocThreats) {
    const iocMetadata = iocThreat.metadata as { cve?: string; vulnerabilityId?: string; packageName?: string } | undefined;
    ...
    const existingThreat = existingThreats.find(t => {
      ...
      return tMetadata?.packageName === iocPkg && 
    ...
    if (existingThreat) {
      ...
      if (iocSeverity > existingSeverity) {
        ...
        existingThreat.confidence = Math.max(existingThreat.confidence || 0, iocThreat.confidence || 0);
