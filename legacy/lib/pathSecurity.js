...
function safeReadDir(dirPath, basePath = process.cwd()) {
  ...
  try {
    ...
    return items.filter(item => {
      ...
      if (VALIDATION_CONFIG.DANGEROUS_FILES.includes(item)) return false;
...
function isAllowedFileType(filePath) {
  ...
  return VALIDATION_CONFIG.ALLOWED_EXTENSIONS.includes(ext);
}
...
function validatePackageName(packageName) {
  ...
  if (!VALIDATION_CONFIG.VALID_PACKAGE_NAME.test(packageName)) {
